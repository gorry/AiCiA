...
public class ActivityExAppList extends PreferenceActivity  {
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		final int pref = R.xml.pref_exapplist;
		...
		for (int i=0; i<SystemConfig.maxExApp; i++) {
			...
			if (p != null) {
				...
				final String exAppName = SystemConfig.exAppName[i];
				final String exPackageName = SystemConfig.exAppPackageName[i];
				final String exActivityName = SystemConfig.exAppActivityName[i];
				if ((exAppName == null) || (exAppName.length() == 0) || (exPackageName == null) || (exPackageName.length() == 0) || (exActivityName == null) || (exActivityName.length() == 0) ) {
					//
				} else {
					p.setTitle(exAppName);
				}
			}
		}
	}
	...
	private class preferenceChangeListener implements OnPreferenceChangeListener {
		public boolean onPreferenceChange(final Preference pref, final Object newValue) {
			...
			if (id >= 0) {
				final String s = SystemConfig.exAppName[id];
				if ((s != null) && (s.length() > 0)) {
					pref.setTitle(s);
				} else {
					pref.setTitle(R.string.activitymain_java_exapp_none);
				}
			}
			return true;
		}
