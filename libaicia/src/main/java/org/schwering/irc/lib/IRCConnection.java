...
public class IRCConnection extends Thread {
	...
	private boolean useJisHalfKana = false;
	// private boolean convertJisHalfKana = false;
	 ...
	 * @see #connect()
	 */
	public IRCConnection(final String host, final int[] ports, final String pass, final String nick,
			final String username, final String realname) {
	 ...
	 * @see #connect()
	 */
	public IRCConnection(final String host, final int portMin, final int portMax, final String pass,
			final String nick, final String username, final String realname) {
	...
	private static int[] portRangeToArray(int portMin, int portMax) {
		if (portMin > portMax) {
			final int tmp = portMin;
			portMin = portMax;
			portMax = tmp;
		}
		final int[] ports = new int[portMax - portMin + 1];
	 ...
	 * @see #run()
	 */
	protected void prepare(final Socket s) throws IOException {
		if (s == null)
			throw new SocketException("Socket s is null, not connected");
		socket = s;
		level = 1;
		s.setSoTimeout(timeout);
		if (encoding.equalsIgnoreCase("ISO-2022-JP_with_halfkana")) {
			useJisHalfKana = true;
			// convertJisHalfKana = false;
			in  = new BufferedReader(new InputStreamReader(s.getInputStream(), "ISO-8859-1"));
			out = new PrintWriter(new OutputStreamWriter(s.getOutputStream(), "ISO-8859-1"));
		} else {
			useJisHalfKana = false;
			// convertJisHalfKana = (encoding.equalsIgnoreCase("ISO-2022-JP"));
	 ...
	 */
	public void send(final String lin) {
		try {
			String line = null;
			if (useJisHalfKana) {
				line = JISUtility.convUnicode2Jis(lin);
			} else {
				line = lin;
			}
			out.write(line +"\r\n");
			...
			if (level == 1) { // not registered
				final IRCParser p = new IRCParser(line);
	 ...
	 */
	private synchronized void get(final String lin) {
		String line = null;
		if (useJisHalfKana) {
			line = JISUtility.convJis2Unicode(lin);
		} else {
			line = lin;
		}
		IRCParser p;
		...
		final String command = p.getCommand();
		int reply; // 3-digit reply will be parsed in the later if-condition

		if (command.equalsIgnoreCase("PRIVMSG")) { // MESSAGE

			final IRCUser user = p.getUser();
			final String middle = p.getMiddle();
			final String trailing = p.getTrailing();
			for (int i = listeners.length - 1; i >= 0; i--)
				listeners[i].onPrivmsg(middle, user, trailing, line);

		} else if (command.equalsIgnoreCase("MODE")) { // MODE

			final String chan = p.getParameter(1);
			if (IRCUtil.isChan(chan)) {
				final IRCUser user = p.getUser();
				final String param2 = p.getParameter(2);
				final String paramsFrom3 = p.getParametersFrom(3);
				for (int i = listeners.length - 1; i >= 0; i--)
					listeners[i].onMode(chan, user,
							new IRCModeParser(param2, paramsFrom3));
			} else {
				final IRCUser user = p.getUser();
				final String paramsFrom2 = p.getParametersFrom(2);
				for (int i = listeners.length - 1; i >= 0; i--)
			...
			final String ping = p.getTrailing(); // no int cause sometimes it's text
			if (pongAutomatic)
				doPong(ping);
			else
				for (int i = listeners.length - 1; i >= 0; i--)
			...
			final IRCUser user = p.getUser();
			final String changingNick = p.getNick();
			final String newNick = p.getTrailing();
			...
			final IRCUser user = p.getUser();
			final String chan = p.getParameter(1);
			final String msg = p.getParameterCount() > 1 ? p.getTrailing() : "";
			...
			final String potNick = p.getParameter(1);
			...
			final IRCUser user = p.getUser();
			final String param1 = p.getParameter(1);
			final String param2 = p.getParameter(2);
			final String msg = (p.getParameterCount() > 2) ? p.getTrailing() : "";
			for (int i = listeners.length - 1; i >= 0; i--)
			...
			final String prefix = p.getPrefix();
			final String middle = p.getMiddle();
			final String trailing = p.getTrailing();
			for (int i = listeners.length - 1; i >= 0; i--)
	 ...
	 *                                  <code>null</code>.
	 */
	public synchronized void addIRCEventListener(final IRCEventListener l) {
		if (l == null)
			throw new IllegalArgumentException("Listener is null.");
		final int len = listeners.length;
		final IRCEventListener[] oldListeners = listeners;
	 ...
	 */
	public synchronized boolean removeIRCEventListener(final IRCEventListener l) {
		if (l == null)
			return false;
		int index = -1;
		...
		final IRCEventListener[] newListeners = new IRCEventListener[len];
	 ...
	 *               colors.
	 */
	public void setColors(final boolean colors) {
	 ...
	 */
	public void setPong(final boolean pong) {
		pongAutomatic = pong;
	}
	 ...
	 */
	public void doAway(final String msg) {
		send("AWAY :"+ msg);
	}
	 ...
	 */
	public void doKick(final String chan, final String nick) {
		send("KICK "+ chan +" "+ nick);
	}
	 ...
	 */
	public void doKick(final String chan, final String nick, final String msg) {
	 ...
	 */
	public void doList(final String chan) {
		send("LIST "+ chan);
	}
	 ...
	 */
	public void doPart(final String chan) {
		send("PART "+ chan);
	}
	 ...
	 */
	public void doPart(final String chan, final String msg) {
		send("PART "+ chan +" :"+ msg);
	}
	 ...
	 */
	public void doPong(final String ping) {
		send("PONG :"+ ping);
	}
	 ...
	 */
	public void doTopic(final String chan) {
		send("TOPIC "+ chan);
	}
	 ...
	 */
	public void doTopic(final String chan, final String topic) {
	 ...
	 */
	public void doWho(final String criteric) {
		send("WHO "+ criteric);
	}
	 ...
	 */
	public void doWhois(final String nick) {
		send("WHOIS "+ nick);
	}
	 ...
	 * @param nick The nickname of the user the query is refering to.
	 */
	public void doWhowas(final String nick) {
		send("WHOWAS "+ nick);
	}
	 ...
	 * @param nick The nickname of the user the query is refering to.
	 */
	public void doUserhost(final String nick) {
		send("USERHOST "+ nick);
	}
	 ...
	 */
	public void doOper(final String user, final String password) {
